...
helm install launchpad ./infra/helm/launchpad \
...
helm install launchpad ./infra/helm/launchpad \
  ...
  --values ./infra/helm/launchpad/values-staging.yaml \
  --create-namespace
...
helm install launchpad ./infra/helm/launchpad \
  ...
  --values ./infra/helm/launchpad/values-production.yaml \
  --create-namespace

# Or use the helper script
./scripts/k8s-deploy.sh production launchpad-production
```
...
helm upgrade launchpad ./infra/helm/launchpad \
  --namespace launchpad-development \
...
- For local development with Kind, use the helper script `./scripts/k8s-setup-kind.sh` to set up the cluster
